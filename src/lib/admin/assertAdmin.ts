...
/** Igual que `assertSuperadminOrThrow`, pero permite admin/superadmin. */
export async function assertAdminOrThrow(req: Request): Promise<Ok | Fail> {
  ...
  const isAdminOrSuper = rawRol === "superadmin" || rawRol === "admin";
  if (!isAdminOrSuper) {
