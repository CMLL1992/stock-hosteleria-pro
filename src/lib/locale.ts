...
export async function loadMessages(locale: Lang): Promise<Record<string, string>> {
  ...
  if (locale === "en") return (await import("@/messages/en.json")).default as Record<string, string>;
  if (locale === "ca") return (await import("@/messages/ca.json")).default as Record<string, string>;
  return (await import("@/messages/es.json")).default as Record<string, string>;
}
